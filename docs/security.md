...
Platform support administrators do not receive permanent company memberships. They must select one company, provide a support reason, and receive a database-enforced grant lasting no more than 60 minutes (30 minutes in the UI). Starting a new grant revokes prior grants. Both the tenant audit log and an append-only platform audit log record access. Production should require phishing-resistant MFA and periodically review every support grant.
