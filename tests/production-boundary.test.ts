...
describe("production data boundary",()=>{
  ...
  it("requires server-verified, reasoned support access",()=>{const route=read("app/api/support/access/route.ts");expect(route).toContain("reason:z.string().trim().min(10)");expect(route).not.toContain("userId");const repository=read("lib/data/manager-repository.ts");expect(repository).toContain("requirePlatformAdmin");expect(repository).toContain("platform_support_grants");});
});
